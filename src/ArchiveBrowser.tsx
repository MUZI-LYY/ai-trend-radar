...
export default function ArchiveBrowser({dates,value,onChange,title,note,buttonLabel}:{title?:string;note?:string;buttonLabel?:string;dates:{date:string;file:string}[];value:string;onChange:(date:string)=>void}){
 ...
 return <details className="archive-browser"><summary>{title??'已存快照 · 按年 / 月 / 周 / 日查找'}</summary><div className="archive-fields">
 ...
 <button onClick={()=>onChange(chosen)}>{buttonLabel??'查看此日归档'}</button><button onClick={()=>onChange('latest')}>返回最新</button>
 </div><p>{note??'按实际归档日期分组，只列出已有快照；榜单统计截止日单独展示。跨月周可在对应月份查看。'}</p></details>
}
