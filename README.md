...
历史入口分为“历史回溯”和“已存快照”，都按年、月、周、日查找。本轮优先补齐 2026-01-01 至 2026-09-13 的 256 个来源日期，收录从 106 扩至 600 个项目，共 142,114 条 2026 年官方日统计；600 项均从年初或创建日起连续覆盖。原有 74 项的 2025 年历史仍保留（23,955 条），所有年份合计 166,069 条。历史入口按年显示有数据项目数、完整覆盖项目数及日统计条数。回溯页依据这些日值重算当前收录项目的日、周、月、年增长；只纳入所选日期已经创建的仓库，完整周期缺失时不参榜。回溯使用当前分类与介绍，存在当前收录范围偏差，不声称是当时的 GitHub 全量榜或本站真实快照；当时累计 Star 留空，增长同分按仓库名排序，也不计算实际在榜天数。
...
人工整理内容保存在 `data/editorial.json`，自动采集优先保留这些修正。未来新发现项目先通过规则和 README 提供初步介绍、分类及标签，并标记为待核实；当前没有接入 LLM 自动生成中文摘要。本轮其余 464 项提供按 README 功能、条件、上手及限制段落整理的原文说明，保留出处，明确区别于中文编辑解读。另有 317 项用途分类覆盖保存在 `data/classification-2026.json`；只核对分类不会把说明标成人工精编。维护者核对官方材料后可补充编辑记录，再运行：
...
python3 -B scripts/collect.py --limit 700 --new-limit 12 --no-discover --refresh
```
...
Actions 保留更新所有已收录项目，新增收录容量为 700 个，每轮最多尝试 12 个候选。发现与采集分开：24 个主题按 Star 分段、排序及分页游标轮转，候选按稳定 ID 去重，队列和失败重试日期持久保存在 `data/discovery.json`。达到收录容量后仍维护发现队列，已有项目继续更新。
